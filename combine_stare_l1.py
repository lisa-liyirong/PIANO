...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 13 15:03:23 2019
Combine level1 stare data to daily files
@author: maren
"""
import numpy as np
import xarray as xr
...
for lidar_str in lidars_str:
    ...
    path_l1_lidar_in=os.path.join(path_l1,'%s_stare_l1_hourly' %lidar_info.name)
    path_l1_lidar_out=os.path.join(path_l1,'%s_stare_l1' %lidar_info.name)
    
    if not os.path.exists(path_l1_lidar_out):
        os.makedirs(path_l1_lidar_out)
    ...
    for yyyymmdd_temp in path_l0_lidar_yyyymmdd:
        ...
        file_out = '%s_stare_%s_l1.nc' %(lidar_info.name,yyyymmdd_temp)
        ...
        dataset_temp.description = ds_temp.description + '; stare files combined for one day'
